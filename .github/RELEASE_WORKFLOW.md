...
This document explains how to use the automated release workflow for the cleared project.
...
You can also create a tag manually and push it to trigger publishing:
...
**Note:** When using tags, make sure the version in `VERSION` and `pyproject.toml` matches the tag.
...
3. **Bump version** - Uses `task version-bump` to update version
4. **Run CI checks** - Runs full test suite (`task ci-local`)
